...
| 6  | 劫持 native 执行路径（hook libc 感知面符号/inline patch 雷管函数入口/驻留匿名信号 handler）；GG 修改器类 ptrace 扫描                                 | 自我 hook 审计三线（GOT/PLT 越界 + 入口序言模式 + 信号 handler 归属，DuckDetector 方法论——检测劫持效果而非注入痕迹，与载体隐蔽方式无关）+ TracerPid 1s 快轮询 + PR\_SET\_DUMPABLE=0；检测与兜底引爆全在 native（Java 层被接管仍工作，引爆=覆写密文+SIGKILL）                                                                          | guard.cpp audit\_execution\_paths（watchdog 自主线程）；GuardManager.checkNow→DefenseProtocol 完整销毁                                                                                 |
...
| 22 | LSPosed（LSPlant）对 Java 关键方法的 call-through hook：注入模块经 ZN 匿名/builtin-linker 装载（maps 无文件路径），Java 层执行路径被桥接，DK/凭据在桥帧窥视下流经 | 栈流审计（fp 链活跃帧 × 敌意可执行区归属）：匿名/撞名 ELF 装载段与无名 exec 跳板池判敌意（真 JIT 按 memfd 命名 + 无 ELF 头豁免，ELF 头邻接判定分撞名），call-through 桥帧是安全链路执行期间的活跃祖先帧、其 RA 必落敌意区——不依赖检测注入框架存在，与装载隐蔽方式无关。检查点 (a) jni\_ct\_eq 内嵌（命中 native 直接引爆）；(b) DK 组装 (c) 敏感读写 (d) 配置下发（命中完整销毁 + fail-closed） | guard.cpp collect\_hostile\_exec\_regions/audit\_call\_stack\_native；GuardManager.auditCallStack；KeyVault.assembleDaemonKey / SensitiveStore / DaemonManager.syncConfig 检查点 |
...
* **GuardManager**：libmemsys.so 的 Java 桥（init/checkNow/auditCallStack/常量时间比较）

* **guard.cpp**（cpp/ 目录）：watchdog、自我 hook 审计三线（GOT/PLT/序言/信号 handler）、栈流审计（敌意可执行区分类 + fp 链活跃帧归属，检查点 a 内嵌引爆）、TracerPid 轮询、自完整性、双实现比较
...
* **短路 hook 不中**：hook 直接返回伪造值、不调用原方法时无桥帧。
  但短路拿不到真 DK/真密文（dk\_check 与敏感读取 fail-closed），
  残余暴露 = 纯参数窃听（与三线时代声明一致）

* **检测是瞬态的**：仅在关键链路（门禁比较/DK 组装/敏感读写/配置
  下发）执行瞬间审计，非连续监控——检查点外的 hook 存活窗口保留

* **帧指针链依赖**：链断裂（omit-fp 编译单元/协程切换边界）就地
  终止 → 只漏报不误报；ART managed 帧与 NDK native 帧在四 ABI
  均维护 fp 链（ART 自身 StackVisitor 同样依赖）

* **OEM 无名真 JIT 理论可误爆**：极少数 ROM 的 ART prctl 命名失败
  产生无名 exec 的真 JIT 段 → 按敌意处理。零售真机 non-hooked
  基线验证是发布前置项（跨 OEM）

* **敌意区分类依赖 maps 真实性**：与三线共享边界，自完整性校验
  独立覆盖

* 端到端测试（宿主 Linux 复现分类器 + fp 链原码）：S0 纯函数
  判定 / S1 干净栈不误杀 / S2 无名匿名跳板命中 / S3 真 JIT 豁免 /
  S4 memfd 撞名 ELF 命中 / S5 匿名 ELF（ZN 主判据）命中 / S6 死帧
  免疫 / S7 邻接前驱 ELF 头命中——全部通过
