...
| 7  | 胁迫场景（被逼交出密码）                                                                                           | 双密码门禁：胁迫验证项命中 → vault **就地重生**（新随机 DK 以缓存的 unwrap 派生键包裹覆盖旧包裹——keySalt 原样保留，文件三 salt 依旧互异、无重生痕迹 = 旧 DK 立即密码学死亡、历史密文孤儿化，Java 层被拦截也已完成；会话以新 DK 全功能续演——演出：该密码正常解锁，同会话新建凭据/启动共享不穿帮）→ 返回 COERCION 由 Java 侧执行销毁序列（演出路径 keepVaultSession 两阶段：擦除关键路径 \~0.5s 本地操作同步完成 + 停共享/停 daemon 后台定向清理并行——vault 层三路径恒定 2 次 Argon2id（见 #11 时序零差），残余差仅一次 185B 原子写，**解锁耗时与正常解锁不可区分**，慢解锁本身即穿帮信号；默认共享密码兜底先于闩锁解除——缺位窗口内竞态 toggle 会以"未配置=无鉴权"拉起 server）——双层独立引爆；界面无任何区分；验证项跨重生存留（门禁行为前后一致，事后取证只见 LIVE\_PW 185B）；代价：原安全密码失效（DK 包裹已易主胁迫密码，用户经 MIGRATE 重设） | vault.cpp UNLOCK 胁迫分支 + rebirth（缓存键）；DefenseProtocol.destroyForCoercion(keepVaultSession)；GatePage；ScreenShareManager 闩锁双轨                  |
...
| 11 | 在线爆破门禁密码（被扣押设备上交互式试密码，含经改密/解除路径旁路）；侧面探测"是否配置胁迫密码"（文件结构/长度/时序）；按解锁耗时区分密码类型（安全/错误/胁迫）                    | UNLOCK/MIGRATE/REMOVE\_GATE 在线限速（共享计数）：连续 5 次失败起指数退避（1s,2s,4s…封顶 60s），成功复位；退避窗内返回码与密码错一致且同样付 2 次哑派生（不暴露限速存在）；计数器仅驻内存（vault 被杀重启即复位，但每次尝试本身要付 64MiB Argon2id，重杀重启的攻击成本不低于等待退避）；**恒定结构密钥文件（v2）**：胁迫验证项恒写（未设置为 rand\_bytes 直填哑项，与真项计算不可区分），无标志字节、长度恒定（LIVE\_PW=185/DEAD\_PW=109），解析严格（r.left==0 否则 CORRUPT）——存在性三通道（结构/长度/时序）零信息，COE 是否配置只存在于用户记忆中；**解锁时序零差**：LIVE\_PW 三路径（安全=unwrap+陪跑 / 错误=unwrap 失败+哑项实测 / 胁迫=unwrap 失败+实测+缓存键重生）恒定 2 次 Argon2id——解锁耗时本身不构成"密码类型"信道                                                          | vault.cpp recordFailure/resetFailure/tryCoe（恒定结构）；opUnlock（等时陪跑+缓存键）；writeLivePw（哑项生成）                                                      |
...
| 14 | 无头销毁路径（Boot/AlarmReceiver goAsync \~10s 预算）被挂起拖垮：root 设备 exec 卡 su 授权弹窗数十秒 → 进程在删 Keystore 前被广播 ANR 杀掉 | 销毁进程清理三项并行（停共享/停 overlay/停 daemon，各有界 3s/3.5s——串行 6.5s 压缩到 \~3.5s，\~10s 预算内到达 Keystore 删除的余量翻倍），超时后台继续、序列推进到 Keystore 删除（密码学销毁优先于进程清理）                                                                                                                                                                                                                                                                                                                                                                                       | DefenseProtocol.runBounded/coroutineScope                                                                                                   |
...
* **DefenseProtocol**：统一销毁入口（两阶段：全量 = 清理三项并行 → 擦除；演出 = 擦除关键路径先行 + 清理后台并行）+ 检查/销毁互斥锁 + init 汇聚点（幂等）
...
* **vault.cpp**（cpp/ 目录，打包 libsyncsvc.so）：DK 唯一持有者——Argon2id 验证（消灭比较点）、DK 包裹落盘、seal/open 流式加密、CK 派生（HMAC-SHA256(DK,"ScreenshotFaker/channel/v1")，DK 永不出 vault）、UNLOCK 限速、胁迫就地重生（rebirth）
...
* 文件 `filesDir/sync_key.bin`（vault 拥有）：状态机
  `[0]'K' [1]ver=2 [2]state`，state ∈ LIVE\_PW/LIVE\_WK/DEAD\_PW
  （详见 vault.cpp 头注释的逐字段格式）；tmp+rename 原子写，
  `sync_key.bin.tmp` 是事务临时态；**恒定结构是冻结不变量**：
  胁迫验证项恒写（未设置为计算不可区分的哑项）、长度恒定
  （LIVE\_PW=185/DEAD\_PW=109/LIVE\_WK=79）、解析严格
  （r.left==0 否则 CORRUPT）、解锁时序零差（LIVE\_PW 三路径恒定
  2 次 Argon2id：安全路径陪跑 / 胁迫路径缓存键重生）——任何未来
  改动不得重新引入 COE 存在性编码（标志字节/可变长度/时序差）
...
* 销毁步骤顺序（见 DefenseProtocol 头注释）；vault 验证项跨销毁/
  重生存留（门禁行为前后一致）；胁迫解锁 = 就地重生（换钥不删
  验证项、状态恒 LIVE\_PW），OP\_DESTROY = DEAD\_PW——两条路径
  语义不得互换
...
销毁不可逆（删 Keystore 条目 + 全部密文）——这句话对无 root
快照能力者是完整的；对持有"销毁前开场快照"的 root：**密封是
保证（他们解不开），销毁是 best-effort（他们留有副本，文件
回滚可恢复密封态）**。以下行为会触发它——
...
* **输入胁迫密码**：门禁页输入胁迫密码 = vault 立即就地重生（旧 DK
  密钥死亡、历史密文孤儿化）+ 完整销毁序列（界面无任何区分，这是
  防胁迫的核心设计；重生会话全功能可用——演出：该密码正常解锁）。
  忘记自己设过胁迫密码而误输入 = 误毁，无法撤销；**重生后原安全
  密码失效**（DK 包裹已易主胁迫密码），恢复使用需以胁迫密码进入
  后 MIGRATE 重设。
...
* **延迟收割（烧毁设备纪律）**：胁迫期被植入的无声键盘记录器/
  hook（root 下不可检测——键盘记录器连检测理论都不存在）可
  等待用户获释后正常解锁时收走 SEC，"未交代密码"的安全承诺
  只在"永不再在此设备解锁"时成立。**经历过胁迫事件的设备视为
  烧毁**：获释后不在该设备输入任何密码（数据对该用户同样
  不可恢复——这是胁迫防御的接受代价，与"无恢复途径"哲学一致）。
  重生式胁迫销毁下，获释后该设备的工作密码即胁迫密码（原安全
  密码已随重生失效）——继续使用等于在已烧毁设备上输入密码，
  违反本纪律

* **重生式胁迫销毁的取证边界**：重生改写 DK 包裹段而验证项照抄、
  文件长度不变——持"胁迫前后快照"的实时 root 可识别"换钥未换
  验证器"（推断胁迫发生过）。旧实现暴露更多（文件 185→109 长度
  跳变 + 销毁后功能全废更易识破），且 Java 侧 DataStore 清扫对
  实时 root 本就不可隐藏——演出针对无快照能力的现场胁迫者；
  事后取证者只见 LIVE\_PW 185B，与从未被胁迫的设备不可区分
