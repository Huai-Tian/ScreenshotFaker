...
| 7  | 胁迫场景（被逼交出密码）                                                                                           | 双密码门禁：胁迫验证项命中 → vault **就地重生**（新随机 DK 以胁迫密码包裹覆盖旧包裹 = 旧 DK 立即密码学死亡、历史密文孤儿化，Java 层被拦截也已完成；会话以新 DK 全功能续演——演出：该密码正常解锁，同会话新建凭据/启动共享不穿帮）→ 返回 COERCION 由 Java 侧执行销毁序列（演出路径 keepVaultSession 两阶段：擦除关键路径 \~0.5s 本地操作同步完成 + 停共享/停 daemon 后台并行——**解锁耗时与正常解锁不可区分**，慢解锁本身即穿帮信号；默认共享密码兜底先于闩锁解除——缺位窗口内竞态 toggle 会以"未配置=无鉴权"拉起 server）——双层独立引爆；界面无任何区分；验证项跨重生存留（门禁行为前后一致，事后取证只见 LIVE\_PW 185B）；代价：原安全密码失效（DK 包裹已易主胁迫密码，用户经 MIGRATE 重设） | vault.cpp UNLOCK 胁迫分支 + rebirth；DefenseProtocol.destroyForCoercion(keepVaultSession)；GatePage；ScreenShareManager 闩锁双轨                       |
...
| 11 | 在线爆破门禁密码（被扣押设备上交互式试密码，含经改密/解除路径旁路）；侧面探测"是否配置胁迫密码"（文件结构/长度/时序）                                          | UNLOCK/MIGRATE/REMOVE\_GATE 在线限速（共享计数）：连续 5 次失败起指数退避（1s,2s,4s…封顶 60s），成功复位；退避窗内返回码与密码错一致（不暴露限速存在）；计数器仅驻内存（vault 被杀重启即复位，但每次尝试本身要付一次 64MiB Argon2id，重杀重启的攻击成本不低于等待退避）；**恒定结构密钥文件（v2）**：胁迫验证项恒写（未设置为 rand\_bytes 直填哑项，与真项计算不可区分），无标志字节、长度恒定（LIVE\_PW=185/DEAD\_PW=109），解析严格（r.left==0 否则 CORRUPT）——存在性三通道（结构/长度/时序）零信息，COE 是否配置只存在于用户记忆中                                                                                 | vault.cpp recordFailure/resetFailure/tryCoe（恒定结构）；writeLivePw（哑项生成）                                                                         |
...
* 文件 `filesDir/sync_key.bin`（vault 拥有）：状态机
  ...
  （r.left==0 否则 CORRUPT）——任何未来改动不得重新引入
  COE 存在性编码（标志字节/可变长度/时序差）
